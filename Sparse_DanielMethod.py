...
########## SPARSE METHOD!!!!####
# ========================== definitions Dim=1 ======================
g = csr_matrix([0, 1])  # ground state
# print("ground=\n", g)

r = csr_matrix([1, 0])  # rydberg state
# print("rydberg=\n", r)

X_i = csr_matrix([[0, 1], [1, 0]])  # flips ground -> rydberg/ rydberg-> ground |g><r|+|r><g|
# print("X_i = \n", X_i)

Q_i = csr_matrix([[1, 0], [0, 0]])  # projector on rydberg state (density of excited states) |r><r|
# print("Q_i = \n", Q_i)

P_i = csr_matrix([[0, 0], [0, 1]])  # projector on ground state (density of ground states) |g><g|
# print("P_i = \n",P_i)

Z_i = csr_matrix([[1, 0], [0, -1]])  # pauli Z


# ======================== declarations of some operators in D=2**n Hilbert (Kronecker) space ===========================

def Z_1(n):
    Z_1 = kron(Z_i, identity(2**(n-1)))
    return Z_1
# print(Z_1(3))

def Z_generali(n,i): # calculates Z for general i
    Z_geni = kron(identity(2**(i-1)),kron(Z_i, identity(2**(n-i))))
    return Z_geni
# print(Z_1(3))

def O_z(n): #mean of Z_i's
    d = 2 ** n
    O_zsum = csr_matrix((d,d))
    for j in range(0,n):
        z_i= kron(kron(identity(2**j),Z_i), identity(2**(n-(j+1))))
...
def NeelHaar(n_tot, n_pxp): #Combination of the Neel and Haar states
    ...
    :return:
    """
    NeelHaarstate = kron(Neelstate(n_pxp), Haarstate(n_tot - n_pxp))
...
def PXP_gen2(n):
    P= csr_matrix([[0, 0], [0, 1]])
    X= csr_matrix([[0, 1], [1, 0]])
    PXP= kron(P,kron(X,P))
    res=csr_matrix((2**n,2**n))
    for i in range(n-2):
        res+=kron(identity(2**(i)),kron(PXP,identity(2**(n-i-3))))
    return res

def PXPOBCNew(n):  # OBC PXP HAMILTONIAN
    d = 2 ** n
    pxp_fin = csr_matrix((d, d))
    for i in range(0, n):  # goes over all atoms (i [from 0 to n-1]= the atom) for total sum in the end
        ...
        if i == 0:
            piminus1 = identity(d)  # boundary (X_1*P_2)
        else:
            piminus1 = kron(identity(2 ** (i - 1)),
                               kron(piminus1, identity(2 ** (n - i))))  # general P_i-1 term
        xi = kron(identity(2 ** (i)), kron(xi, identity(2 ** (n - (i + 1)))))  # general X_i term
        if i == n - 1:
            piplus1 = identity(d)  # boundary (P_N-1*X_N)
        else:
            piplus1 = kron(identity(2 ** (i + 1)),
                              kron(piplus1, identity(2 ** (n - (i + 2)))))  # general P_i+1 term
        pxp_ar = piminus1.multiply(xi.multiply(piplus1))  # calculates hamiltonian PER i
        pxp_fin +=pxp_ar # cumulative sum over i
    return pxp_fin
...
def TIOBCNew(n_TI, h_x, h_z): # Tilted Ising Hamiltonian OBC n= no of atoms (n must be =>2)
    ...
    """
    d = 2 ** n_TI
    TI_fin = csr_matrix((d, d))
    for i in range(0, n_TI):
        zi = Z_i  # inital declaration
        ziplus1 = Z_i  # inital declaration
        xi = X_i  # inital declaration
        zi = kron(identity(2 ** i), kron(zi, identity(2 ** (n_TI - (i + 1)))))  # Z_i term
        if i == n_TI - 1:
            ziplus1 = csr_matrix((d, d))  # Z_i+1 boundary (kills boundary)
        else:
            ziplus1 = kron(identity(2 ** (i + 1)),
                              kron(ziplus1, identity(2 ** (n_TI - (i + 2)))))  # Z_i+1 term
        xi = kron(identity(2 ** (i)), kron(xi, identity(2 ** (n_TI - (i + 1)))))  # X_i term
...
def TIOBCNewImpure(n_TI, h_x, h_z, h_i): #same Tilted Ising only with impurity at the Z_1 site!!
    ...
    for i in range(0, n_TI):
        ...
        TI_ar = np.add(np.add(np.matmul(zi, ziplus1), (h_z) * zi), (h_x) * xi)  # calculates hamiltonian PER i
        TI_fin = np.add(TI_fin, TI_ar)
    TI_fin_impure= np.add(TI_fin, h_i*kron(Z_i,identity(int(np.divide(d,2)))))
...
def Coupling(n_tot, n, Coupmat, h_c): # 2 site coupling matrix in TOTAL Hamiltonian size (2**n_totx2**n_tot )
    ...
    d_TOT = 2 ** n_tot
    Couplmat = (h_c) * kron(Coupmat,Coupmat)
    if np.subtract(n_tot,n) == 0 or np.subtract(n_tot,n) == n_tot or h_c == 0:
        Coupterm = csr_matrix((d_TOT,d_TOT))
    else:
        Coupterm = kron(kron(identity(2 ** (n - 1)), Couplmat), identity(2 ** (np.subtract(n_tot, n)-1)))
    return Coupterm
...
def Couplingalt(n_tot, n, Coupmat, h_c): #alternative way to write coupling (slower!!!, the older one)
    ...
    if np.subtract(n_tot,n)==0 or np.subtract(n_tot,n)==n_tot or h_c==0:
        ...
        CoupMat_n = kron(kron(identity(2 ** (n - 1)), Couplmat), identity(d_TI))
        CoupMat_nplus1 = kron(kron(identity(d_pxp), Couplmat), identity(2 ** (n_tot - (n + 1))))
...
#     HamNoCoup = np.add(kron(PXP, identity(d_TI)), kron(identity(d_pxp), TI))
...
def EigenSpanAlt(Mat,
    ...
    Eval, Evec = EvecEval(Mat)
    Z_2 = VecState
    y = csr_matrix(np.size(Eval)).astype('complex')
    for j in range(0, np.size(Eval)):
...
def RMeanMetric(EV):  #Mean R metric, r= 0.39 poisson, r=0.536 W-D
    ...
    for i in range(1, S.shape[0]):
        r = r + np.divide(min(S[i], S[i - 1]), max(S[i], S[i - 1])) #???out=csr_matrix((1)), where=max(S[i], S[i - 1]) != 0)
        #print(max(S[i], S[i - 1]))
