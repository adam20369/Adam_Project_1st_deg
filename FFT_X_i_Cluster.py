...
def Cluster_Realizations_FFT(n_PXP, n_TI, h_c, T_start, T_max, T_step, Height_norm=1):
    ...
    if os.path.isfile('PXP_{}_TI_{}_X_i/h_c_{}/Fourier_components_X_i_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c,n_PXP,n_TI,h_c))==False: #checks if file already exists, can take this part off of code
        for i in range(1,seed_max):
            VecProp = np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Sparse_time_propagation_X_i_{}_{}_{}_sample_{}.npy'.format(n_PXP,n_TI,h_c,i)))
            Fourier_components[i-1,:]= rfft(VecProp)
        np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Fourier_components_X_i_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)), Height_norm * np.abs(Fourier_components))
...
def Cluster_FFT_Freq(T_start, T_max, T_step):
    ...
    if os.path.isfile('PXP_{}_TI_{}_X_i/h_c_{}/Frequency_T_max_{}_T_step_{}.npy'.format(n_PXP,n_TI,h_c,T_max,T_step)) == False:
        np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Frequency_T_max_{}_T_step_{}.npy'.format(T_max,T_step)), Freq)
...
def Cluster_Lorentzian_curvefit(n_PXP, n_TI, h_c, T_start, T_max, T_step, Start_cutoff, End_cutoff): #TODO CHECK IF CURVEFIT SHOULD BE DONE PARRALELISH
    ...
    '''
    Freq= np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Frequency_T_max_{}_T_step_{}.npy'.format(T_max,T_step)))
    sig_func = np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Fourier_components_X_i_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c))) #matrix!
    ...
    for i in range(1,seed_max):
        ...
        popt_tot[i-1,:]=popt
    np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'gamma_X_i_array_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)), popt_tot[:,1])
...
def Gamma_time_ave():
    ...
    '''
    data= np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'gamma_X_i_array_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)))
    data_ave = np.mean(data)
    np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_ave_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)), data_ave)
...
def Gamma_Bootstrap_confidence(Sample_no):
    ...
    np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_errors_confidence_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)),lower_upper)
...
def Gamma_Bootstrap_std(Sample_no):
    ...
    np.save(os.path.join('PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_errors_std_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)),std)
...
def FFT_data_move():
    ...
    '''
    data_ave = np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_ave_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)))
    data_err_std= np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_errors_std_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)))
    data_err_confidence = np.load(os.getcwd()+os.path.join('/PXP_{}_TI_{}_X_i/h_c_{}'.format(n_PXP,n_TI,h_c),'Gamma_X_i_errors_confidence_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)))
    try:
        os.mkdir('PXP_{}_X_i_Gammas'.format(n_PXP))
    except:
        pass
    np.save(os.path.join('PXP_{}_X_i_Gammas'.format(n_PXP),'Gamma_X_i_ave_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)),data_ave)
    np.save(os.path.join('PXP_{}_X_i_Gammas'.format(n_PXP),'Gamma_X_i_errors_std_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)),data_err_std)
    np.save(os.path.join('PXP_{}_X_i_Gammas'.format(n_PXP),'Gamma_X_i_errors_confidence_{}_{}_{}.npy'.format(n_PXP,n_TI,h_c)),data_err_confidence)
