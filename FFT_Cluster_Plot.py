...
def Plot_vs_h_c_diff_n_TI_Rel_Err_new():
    ...
    for j in np.nditer(n_TI):
        for i in np.nditer(h_c):
            data_ave[int(np.round(i, 1) * 10)] = np.load(
                'PXP_{}_Gammas_cutoff_{}_{}/Gamma_ave_{}_{}_{}_cutoff_{}_{}.npy'.format(n_PXP, Start_cutoff, End_cutoff,
                                                                                        n_PXP, int(j), np.round(i, 1),
                                                                                        Start_cutoff, End_cutoff))
            # print(data_ave)
            data_errors[int(np.round(i, 1) * 10)] = np.load(
                'PXP_{}_Gammas_cutoff_{}_{}/Gamma_error_ave_{}_{}_{}_cutoff_{}_{}.npy'.format(n_PXP, Start_cutoff,
                                                                                               End_cutoff, n_PXP,
                                                                                               int(j), np.round(i, 1),
                                                                                               Start_cutoff,
                                                                                               End_cutoff))
        ...
        plt.fill_between(h_c[:], data_errors_fin, data_errors_fin)
        print(data_ave[11])
    plt.title(r'$Z_i$ coupling normalized damping for {} PXP atoms vs $h_c$, cutoff {}-{}'.format(n_PXP, Start_cutoff,
                                                                                                  End_cutoff))
    plt.ylabel(r' Normalized Damping $\frac{\gamma}{\gamma_0}$', fontsize=10)
    plt.xlabel(r'Coupling Strength $h_c$', fontsize=12)
    plt.legend()
    plt.savefig('Figures/True_X_i_Osc/Z_i_damping_{}_PXP_vs_h_c_{}_Rel_err_new_{}_{}.png'.format(n_PXP, h_c_max,Start_cutoff,End_cutoff))
    return plt.show()
